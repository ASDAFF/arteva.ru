...
// --- костыль для чекаута BEGIN
if ($_REQUEST):
	$_REQUEST["REGISTER"]["EMAIL"] = $_REQUEST["REGISTER"]["LOGIN"];
	if ($_REQUEST["REGISTER"]["CHECKOUT"] == "Y"):
		$_REQUEST["REGISTER"]["PASSWORD"] = time();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]) && !$USER->IsAuthorized())
{
	...
	else // if there;s no any errors - create user
	{
		...
		if (intval($ID) > 0)
		{
			...
			// отправка сообщения при регистрации. Пользователю.
				$arEventFields["URL_LOGIN"] = htmlspecialchars($arEventFields["LOGIN"]);
...
if($register_done)
{
	...
	if ($_REQUEST["REGISTER"]["SUBSCRIBE"] <> ''):
		subscribeRegUser($_REQUEST["REGISTER"]["EMAIL"]);
	endif;
