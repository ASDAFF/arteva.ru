...
//				}
		...
		GetTableOperation: function()
		{
			...
			{
				if (node.nodeName != "TD")
					node = BX.findParent(node, {tag: 'TD'}, table)

				if (node && node.nodeName == "TD" && !BX.util.in_array(node, cells))
					cells.push(node);
			...
			function getSelectedCells(range, table)
			{
				...
				//selectedNode = _this.editor.selection.GetSelectedNode(range);
			...
			{
				var td = BX.findParent(element, {nodeName: 'TD'});
				if (!td || !table)
					return;
			...
			function getMergeState(cells, table)
			{
				...
				for(i = 0; i < cells.length; i++)
				{
					...
					{
						if (rowStamp == '' || BX.util.trim(rowStamp) != ind[j].r)
							rowStamp += ' ' + ind[j].r;

						if (colStamp == '' || BX.util.trim(colStamp) != ind[j].c)
							colStamp += ' ' + ind[j].c;
					}
					rowStamp = BX.util.trim(rowStamp);
					colStamp = BX.util.trim(colStamp);

					if (rInd === undefined)
						rInd = rowStamp;
					else if (rInd !== rowStamp)
						sameRow = false;

					if (cInd === undefined)
						cInd = colStamp;
					else if (cInd !== colStamp)
						sameCol = false;

					if (sameRow && !gaps)
					{
						if (lastCellIndex !== undefined && Math.abs(lastCellIndex - cells[i].cellIndex) > 1)
							gaps = true;
						lastCellIndex = cells[i].cellIndex;
					}

					if (sameCol && !gaps)
					{
						if (lastColIndex !== undefined && Math.abs(lastColIndex - tr.rowIndex) > 1)
							gaps = true;
						lastColIndex = tr.rowIndex;
					}
				}

				return {sameCol : sameCol, sameRow: sameRow, gaps: gaps};
			}
